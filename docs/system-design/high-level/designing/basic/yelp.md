...
Replication Strategies determine how data and state are copied across multiple nodes in Yelp. The choice of strategy determines the trade-off between consistency, availability, and latency.

**Why it matters**

Yelp must replicate data to prevent loss and serve reads from multiple nodes. The replication strategy determines whether the system favors consistency (strong reads) or availability (always writable), and how it handles cross-node failure.
...
|---|---|---|---|
| Leader-based | reviewer identity, review text | Strong consistency, simple | Write bottleneck, leader failure risk |
| Multi-leader | business listings, aggregate ratings | Low-latency global writes | Conflict resolution, complex |
...
Failure Detection and Membership is the mechanism by which Yelp determines which nodes are alive and part of the cluster. Membership is the list of known nodes; failure detection is the process of updating that list when nodes fail or recover.

**Why it matters**

Yelp must route requests only to healthy nodes and trigger failover when a node goes down. False positives (healthy nodes marked as dead) cause unnecessary failovers and data loss. False negatives (dead nodes not detected) cause failed requests and degraded performance.
...
High Availability and Scalability determines how Yelp continues operating when nodes or entire zones fail, and how capacity is added as demand grows. HA is about minimizing downtime; scalability is about maintaining performance as load increases.

**Why it matters**

Yelp must stay operational even when individual nodes, availability zones, or entire regions fail. At the same time, it must scale horizontally to handle traffic spikes without degrading latency. The two are intertwined: the replication strategy, failure detection, and load balancing all contribute to both HA and scalability.
...
3. **Connection draining**: When scaling in, existing connections are allowed to complete before the node is shut down. For Yelp, this means draining active 1. sessions gracefully.
...
Performance and Optimization covers the techniques Yelp uses to achieve low latency, high throughput, and efficient resource usage. This section examines the key performance metrics, bottlenecks, and optimizations specific to the system.

**Why it matters**

Yelp faces competing pressures: users demand low latency, the system must handle high throughput, and infrastructure costs must be controlled. The optimizations applied at the data, compute, and network layers determine whether the system meets its SLA.

**How it works**

**Latency layers**: Latency in Yelp comes from three layers:
...
**Caching strategies**: Yelp uses multiple cache layers:
- **Edge cache**: Static assets (images, CSS, JS) served from a CDN at the edge. For Yelp, this caches business listings, aggregate ratings that doesn't change frequently.
- **Application cache**: In-memory cache (e.g., Redis, Memcached) for frequently accessed data. Cache-aside pattern: application checks cache first, falls back to database on miss.
- **Local cache**: In-process cache (e.g., Caffeine, Guava) for data accessed within a single request. Avoids network round-trips.

**Batching and pipelining**: Yelp batches small operations (e.g., writes, log flushes) to amortize per-operation overhead. Pipelining allows multiple requests to be in flight simultaneously.
...
**Connection pooling**: Yelp maintains a pool of database connections to avoid the TCP handshake overhead on every request. Pool size is tuned to match the database's capacity.

**Compression**: Responses are compressed (gzip, zstd) for clients that support it. At the network layer, gRPC uses HPACK header compression.

**Indexing**: Database tables are indexed on frequently queried fields. For Yelp, indexes cover **Responsibilities**: TLS termination, authentication token validation, rate lim and **How it works**: Stateless tier behind a load balancer; validates JWTs locally  for fast lookups.
...
Encryption and Key Management in Yelp ensures that data is protected both at rest (stored on disk) and in transit (moving between services). Key management governs how encryption keys are generated, stored, rotated, and accessed — without proper key management, encryption provides a false sense of security.

**Why it matters**

Yelp handles reviewer identity, review text that must be encrypted both at rest and in transit. Scaling Yelp to handle increasing load while maintaining data consistency, low latency, and fault tolerance requires careful key management: keys must be rotated regularly, scoped to prevent cross-contamination, and audited for compliance. A single key compromise could expose sensitive data.

**How it works**

**At-rest encryption**: Data stored in **Purpose**: Single entry point for all client traffic (mobile apps, web, partne, **Responsibilities**: TLS termination, authentication token validation, rate lim and databases is encrypted using AES-256-GCM. The Data Encryption Key (DEK) is generated per data partition and encrypted with a Key Encryption Key (KEK) managed by a KMS (AWS KMS, GCP KMS, HashiCorp Vault). Keys are regionally scoped — only data belonging to a region can be decrypted by that region's KEK.

**In-transit encryption**: All inter-service communication uses TLS 1.3 or mTLS for service-to-service auth. Cross-region communication of business listings, aggregate ratings uses TLS + optional application-level encryption. reviewer identity, review text is NEVER transmitted in plaintext or across region boundaries.
...
**Cross-region key sharing**: For non-restricted data (business listings, aggregate ratings), a shared key is imported into each region's KMS. Restricted data NEVER uses cross-region keys — each region's KMS holds only that region's keys.
...
Authentication and Authorization (AuthN/AuthZ) in Yelp control who can access the system and what they can do. Authentication verifies identity; authorization determines permissions. In a distributed system like Yelp, auth must work across multiple nodes while respecting data boundaries — a user authenticated in one region should not have their session data or tokens replicated to regions where it is not permitted.

**Why it matters**

Yelp must verify identity at the edge and enforce authorization at every service boundary. reviewer identity, review text must be protected — only users with appropriate roles should access it. At the same time, business listings, aggregate ratings data should be accessible to a wider audience with minimal friction.
...
- **Resource-level authorization**: Each request is checked against an ACL (Access Control List) that specifies which roles can access which resources. For Yelp, restricted resources require the `admin` role + matching region.
...
Security Threats and Mitigations catalog the attack surface of Yelp, the most likely threats, and the corresponding defenses. Every distributed system has unique threat vectors — Yelp is no exception.

**Why it matters**

Yelp handles reviewer identity, review text that attackers might target. Scaling Yelp to handle increasing load while maintaining data consistency, low latency, and fault tolerance expands the attack surface: more nodes, more network paths, more failure modes. Without proper threat modeling, a single vulnerability could expose sensitive data across the entire system.
...
**Data exfiltration prevention**: Yelp enforces data residency by design — reviewer identity, review text is never replicated cross-region. The replication layer checks a data classification label before allowing cross-region copy. A database-level policy (e.g., PostgreSQL RLS) also blocks cross-region queries for restricted partitions.

**mTLS enforcement**: All service-to-service communication uses mutual TLS. Certificates are issued by a per-region CA and rotated every 30 days. Services must present a valid certificate to communicate — no plaintext connections are allowed.

**PII redaction**: Application logs are scanned for PII patterns (email, phone, credit card) using an automated redaction layer (e.g., AWS Macie, Google DLP). business listings, aggregate ratings is logged freely; restricted fields are masked or dropped before logging.
...
**Audit trail**: All security-relevant events (auth, data access, config changes) are written to an append-only audit log with cryptographic integrity (HMAC per entry). The log covers reviewer identity, review text access — who accessed what, when, and from where.
...
Observability and Logging in Yelp provide visibility into system behavior through three pillars: **metrics** (aggregates and counters), **logs** (structured event records), and **traces** (distributed request timelines). Together, these signals allow operators to debug issues, detect anomalies, and verify SLA compliance.

**Why it matters**

Distributed systems like Yelp are inherently opaque — failures can cascade across services in unexpected ways. Without good observability, a single slow dependency can cause widespread timeouts that are nearly impossible to diagnose. Scaling Yelp to handle increasing load while maintaining data consistency, low latency, and fault tolerance makes observability even more critical: operators need to see cross-region latency, regional failures, and data-residency violations.

**How it works**

**Metrics**: Yelp instruments every service with Prometheus-style metrics:
- **Request rate, error rate, duration (the "RED" metrics)**: Tracks per-endpoint HTTP latency and error counts.
- **System metrics**: CPU, memory, disk I/O, network throughput per node.
- **Business metrics**: For Yelp, this includes metrics like "**Responsibilities**: TLS termination, authentication token validation, rate lim fill rate" and "reservation conflict rate".

Metrics are aggregated in a time-series database (Prometheus, VictoriaMetrics) and visualized in Grafana dashboards with alerts via PagerDuty.

**Logs**: Yelp uses structured logging (JSON format) with standardized fields:
...
reviewer identity, review text access is logged with full context (user, action, resource). business listings, aggregate ratings logs are aggregated and may have reduced retention.

**Distributed tracing**: Every request is assigned a trace ID at the edge. The trace propagates through all downstream services via HTTP headers. A tracing backend (Jaeger, Tempo, Zipkin) reconstructs the full request timeline, showing latency at each hop. For Yelp, traces include region boundaries — a cross-region call is annotated as such.
...
**Alerting**: Yelp defines SLO-based alerts:
...
- **Yelp platforms**: widely used yelp platform
